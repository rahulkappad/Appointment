...
const MyAppointment = () => {
  ...
  const slotDateFormat = (slotDate) => {
    if (!slotDate) return "Invalid Date"; // Handle undefined/null slotDate gracefully
    const dateArray = slotDate.split("_"); // Use slotDate (not item.slotDate)
        ...
        {appointments.map((item,index)=>(
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
              ...
              <div className='flex flex-col gap-2 justify-end'>
                ...
                {item.cancelled && !item.isCompleted && <button className='text-sm text-red-500 text-center sm:min-w-48 py-3 border rounded border-red-500 '> Appointment Cancelled</button>}
                {item.isCompleted && <button className='text-sm text-primary text-center sm:min-w-48 py-3 border rounded border-primary '>Completed</button>}
              </div>
          </div>
        ))}
      </div>
